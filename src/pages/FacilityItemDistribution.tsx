...
import { Plus, Save, Trash2, Building2, FileBarChart } from "lucide-react"
...
import {
  useFacilityCollections,
  type FacilityCollection,
} from "@/hooks/useFacilityCollections"
...
const DISTRIBUTION_EXPORT_TABLE_ID = "facility-item-distribution-export-table"
...
export default function FacilityItemDistribution() {
  ...
  const {
    ...
    facilityId: filterFacilityId || undefined,
  ...
  const saveBatch = async () => {
    if (!storeId) {
      toast({
        title: "Error",
        description: "Please select a source store",
        variant: "destructive",
      })
      return
    }
    ...
    if (!staffId) {
      toast({
        title: "Error",
        description: "Please select a receiving staff member",
        variant: "destructive",
      })
      return
    }
    ...
    toast({
      title: "Saving...",
      description: "Please wait while we save the facility distribution batch",
    })

    try {
      await createBulkFacilityCollections({
      ...
      toast({
        title: "Success",
        description: "Facility distribution recorded successfully",
      })
      setNewItems([])
      setNotes("")
      setReferenceNo("")
      ...
      toast({
        title: "Error",
        description:
          "Failed to record facility distribution: " + (err as Error).message,
        variant: "destructive",
      })
    }
  }
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteFacilityCollection(collectionToDelete)
  ...
  const groupedBatches = facilityCollections.reduce((acc, row) => {
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between print:hidden">
        <div>
          <h1 className="text-3xl font-bold">Facility item distribution</h1>
          <p className="text-muted-foreground">
            Issue inventory from a store to a facility, linked to a receiving staff member
          </p>
        </div>
          ...
          <Card className="overflow-hidden">
            <CardHeader>
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                ...
                <ReportActions
                  tableId={DISTRIBUTION_EXPORT_TABLE_ID}
                  filenameBase="facility-item-distribution"
              ...
              <p className="hidden print:block text-sm text-muted-foreground mt-2">
                Facility item distribution report
              </p>
            </CardHeader>
            <CardContent className="p-0">
              {isLoading ? (
                ...
                <div className="text-center py-8 px-6">
                  ...
                  <p className="text-muted-foreground">
                    Record distributions on the New entry tab or adjust filters.
                  </p>
                </div>
              ) : (
                <>
                <div className="overflow-x-auto">
                <Table id="facility-item-distribution-display-table">
                  <TableHeader>
                    <TableRow>
                ...
                <div className="sr-only" aria-hidden>
                  <Table id={DISTRIBUTION_EXPORT_TABLE_ID}>
                    <TableHeader>
                      <TableRow>
                        <TableHead>Reference</TableHead>
                        <TableHead>Date</TableHead>
